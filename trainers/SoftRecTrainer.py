...
class SoftRecTrainer(AbstractBaseTrainer):
    ...
    def close_training(self):
        # cPickle.dump([self.weight_log, self.item_id, self.epochs], open(os.path.join(self.export_root, 'logs', 'weight_info_log.pth'), "wb"))
        
        # mentor_path = Path(self.export_root).joinpath("mentor_models")

        # if not os.path.exists(mentor_path):
        #     os.mkdir(mentor_path)

        # save_state_dict(self._get_mentor_state_dict(), Path(self.export_root).joinpath("mentor_models"), "best_acc_model.pth")
        pass
    ...
    def _create_optimizer(self):
        ...
        if args.optimizer.lower() == 'adam':
            # return torch.optim.Adam(chain(self.model.parameters(), [self.loss.b]), lr=args.lr, weight_decay=args.weight_decay)
            return torch.optim.Adam(self.model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        elif args.optimizer.lower() == 'sgd':
            # return torch.optim.SGD(chain(self.model.parameters(), [self.loss.b]), lr=args.lr, weight_decay=args.weight_decay, momentum=args.momentum)
